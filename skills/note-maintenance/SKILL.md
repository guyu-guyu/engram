...
### 第一步：同步常驻记忆 → 记忆库（确保库覆盖常驻记忆里的跨任务内容）

**时机：必须在最开头**——本步的 `note_write` 会标脏 group，让这些新条目进入后续整理流程。

**范围（用户 2026-09-21 定的分层规则，先看这条）**：

- **只补写「跨任务」的常驻记忆内容**：环境事实、约定、决策与知识（例如「晚上=20:00」「代理禁 TUN」）。
- **特定领域/任务专属的流程、命令、坑、修法 → 只进 skill，不写记忆库**（用户原话：hermes 的 skill 机制比较完善，进 skill 后就不需要再进记忆库）。碰到这种条目：用 `skill_manage` 把它归位到对应 skill（patch 现有 skill / write_file 支持文件），再从常驻记忆里删掉或精简。
- **一份知识只维护一处**：本步不是「把常驻记忆照抄进库」，而是「把跨任务内容落入库 + 把领域内容归位到 skill」。

1. **读常驻记忆**：`read_file` 读取 `~/.hermes/memories/MEMORY.md` 和 `USER.md`（或直接用 system prompt 中已注入的 MEMORY / USER PROFILE 块）
2. **逐条核对并分类**：对每条内容判断是「跨任务知识」还是「领域/任务专属」；跨任务的用 `note_search` / 对照 INDEX 看库里是否已有等价条目
3. **缺失则写入**：`note_write` 按常规流程添加——选主题最近的组（或新建组）、给有意义的 `entry_header`、组自动标脏
4. **完成判定**：常驻记忆里的**跨任务内容**在库中都有等价条目；领域内容已归位到 skill（不在库里重复）
...
**4.8 与 skill 查重（用户 2026-09-21 指定）**——领域/任务专属内容的家是 skill，不是记忆库：

对脏组里**每一条条目**做一次查重：

1. **判定内容性质**：这条讲的是「特定领域/任务的流程、命令、坑、修法」还是「跨任务知识（环境事实、约定、决策及其验证）」？
2. **领域/任务类 → 去 skill 里查**：用 `skills_list()` + 在 `~/.hermes/skills` 里 grep 关键词/命令名（如 `grep -rn "<命令>|<路径>|<类名>" ~/.hermes/skills`），确认同等内容是否已在某个 skill 里。
3. **重复且无需保留 → 从记忆库删**：该条目本身没有独立价值（它在 skill 里有更完整的版本）时，直接不在 `note_rewrite` 的 entries 里回写（等于删条）。
4. **部分重复 → 只删重复的那部分**：条目里混着跨任务事实时（例：一段讲 time-pipeline 单测命令的领域内容 + 一句「本项目数据管道与看板分开部署」的环境事实），保留跨任务部分，删掉已入 skill 的那段，header 相应改准。
5. **反过来不做**：不要因为记忆库里也有，就去删/改 skill 的内容——skill 是领域内容的权威源；跨任务知识则以记忆库为准。删之前先确认 skill 里确实有等价内容（不确定就保留并 `note_comment` 提问）。
6. **报告里逐条列出**「删了哪条/哪段、对应 skill 在哪里」，保持可追溯。
...
- **❌ 脏组里沉淀着已入 skill 的领域内容而不清理**：按用户 2026-09-21 的规则，处理脏组时每条都要过一遍「这是不是领域/任务专属、skill 里是否已有」——重复且无跨任务价值的条目/片段删掉（详见 4.8），否则同一份知识在两个地方各自腐化
- **❌ 把领域/任务专属内容同时写进 skill 和记忆库**（用户 2026-09-21 明确）：特定领域/任务的流程、命令、坑只维护在 skill 里；记忆库只收跨任务内容（环境/约定/决策/知识）。发现自己要写一条「只在某类任务里才有用」的内容 → 写 skill，不写库；已在库里的这类条目，维护时迁入 skill 并删条
