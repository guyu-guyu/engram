...
- **❌ 把被实测证伪的旧结论改成「已可用 / 已修复」留在库里**——等于用条目去记一件**不存在的事**（实例：把「已知缺陷：CJK 中文搜索失效」改写成「中文搜索已可用」）。正确动作是**让那段文字消失**：直接删掉该段，并清掉 header / 标签里为它保留的关键词；只保留可复用事实（路径 / 参数 / 机制）。也不要在别的条目里复述这段历史（曾在维护复盘条目里又讲了一遍，等于第二条也在记这桩不存在的缺陷）。**库只记「现在是什么」，不记「曾经不是这样」。**
- **❌ 借「顺手修正」把改动范围扩大**——维护是响应式的：第一步核对常驻记忆时读到过时内容，允许做「实测证伪 + 定向单条修正/删除」这类小动作，但不许顺带重组、合并非脏组；每个动作都要能说出明确收益，并写进汇报
