...
class EngramProvider(MemoryProvider):
    ...
    def system_prompt_block(self) -> str:
        ...
        return (
            ...
            "LAYERING RULE (user, 2026-09-21): domain/task-specific "
            "procedures, commands, pitfalls and fixes belong in a SKILL only "
            "— never duplicate them here as well. This repo takes cross-task "
            "knowledge: environment facts, standing conventions, decisions "
            "and their verification. Resident memory keeps only facts that "
            "must hold in every session. Test to apply: would this still "
            "matter on an unrelated task? No → skill, not this repo. "
            "SYNC RULE: writing to session-level memory (Hermes `memory` "
            "tool) REQUIRES a matching note_write to this repository in the "
            "same turn — for cross-task content only; skill-domain content "
            "never lands here, and memory is a subset of this repo. The "
            "reverse does NOT hold: note_write alone is fine without a "
            "memory write. "
            "RESIDENT-MEMORY APPEND RULE: when writing to session-level "
            "memory, the new entry is appended at the END of the resident "
            "memory file (never inserted in the middle or replacing the "
            "structure). If the resident memory is over its character "
            "budget after the append, consolidate by removing the OLDEST "
            "entries first (lowest value / most stale) until it fits — "
            "newest knowledge wins, oldest is evicted. "
